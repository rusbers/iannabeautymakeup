...
export async function generateStaticParams(props: { params: Promise<{ lang: Locale }> }) {
  const { lang } = await props.params
  const pages = await fetchSanityPagesStaticParams({ language: lang })
  return pages.map((page) => ({ slug: page.slug?.current }))
}

export async function generateMetadata(props: { params: Promise<{ slug: string; lang: Locale }> }) {
